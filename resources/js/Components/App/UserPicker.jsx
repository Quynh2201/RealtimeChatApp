                            ...
                            {filteredPeople.length === 0 && query !== "" ? (
                                ...
                                filteredPeople.map((person) => (
                                    <Combobox.Option 
                                        key={person.id} 
                                        value={person} 
                                        className={({ active }) => `relative cursor-default select-none py-2 l-10 pr-4 ${active ? "bg-teal-600 text-white" : "bg-gray-900 text-gray-100"}`}
                                    >
                                        {({ selected, active }) => (
                                            <div className="flex items-center ml-2">
                                                <span className={`block truncate ${selected ? "font-medium" : "font-normal"} mr-2`}>
                                                    {person.name}
                                                </span>
                                                {selected ? (
                                                    <span className="flex-shrink-0">
                                                        <CheckIcon className="h-5 w-5 mr-2 text-white" aria-hidden="true" />
                                                    </span>
                                                ) : null}
                                        </div>
                                        )}
